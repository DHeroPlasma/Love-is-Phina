...
const adminKeyInput =
  document.querySelector("#adminKeyInput");
...
async function loadJoinInfo() {
  try {
    ...
    console.error(
      "Join information could not be loaded:",
      error
    );
  }
}
    ...
    [...participants]
      ...
      .map(
        (participant) => {
          ...
          const answered =
            game?.round === 2
              ? participant.round2Answered
              : participant.round1Answered;
...
function renderQuestions() {
  ...
  title.textContent =
    `Fragen · ${game.roundLabel}`;

  progress.textContent =
    `${game.completedQuestions.length}/${game.totalQuestions} abgeschlossen`;
    ...
    game.questions
      .map(
        (
          ...
          const completed =
            game.completedQuestions.includes(
              index
            );
...
function renderStage() {
  ...
  if (
    ...
    matchingStage.classList.remove(
      "hidden"
    );
  ...
  if (
    ...
    document.querySelector(
      ...
      game.round === 1
        ? "Die Findungsrunde kann beginnen."
        : "Die Pärchenrunde kann beginnen.";
    ...
    game.unmatched
      ? `
        <div class="unmatched-note">
          ...
          <strong>
            ${escapeHtml(
              game.unmatched.realName
            )}
          </strong>

          (
            ${escapeHtml(
              game.unmatched.pseudonym
            )}
          )
...
function renderWinners() {
  ...
  if (
    ...
    title.textContent =
      "Noch kein Gewinnerpaar";

    content.innerHTML =
      "<p>Es konnten keine Paare ausgewertet werden.</p>";
      ...
      .map(
        (pair) => `
          <div class="winner-pair">
            ...
            <span>
              ${pair.round2Agreement}/${game.totalQuestions}
              Übereinstimmungen
            </span>
...
/* -------------------------------------------------------------------------- */
/* Generic socket response handling                                           */
/* -------------------------------------------------------------------------- */
...
document.querySelector(
  ...
  () => {
    ...
    const confirmed =
      window.confirm(
        "Spiel wirklich vollständig zurücksetzen?\n\n" +
        "Dabei werden alle Teilnehmer, Antworten, Matches und Recovery-Daten gelöscht.\n\n" +
        "Alle verbundenen Gäste werden automatisch zurück zur Namenseingabe geschickt."
      );
    ...
    socket.emit(
      ...
      (response) => {
        ...
        /*
          The server already sends game:update and
          participants:update events after the reset.

          We still apply the returned fresh state here
          immediately so the host interface updates
          without waiting for another Socket.IO event.
        */
